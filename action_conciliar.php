...
if ($id_extracto > 0) {
    ...
    if ($mov) {
        ...
        $res_b = $db->query($sql_b);
        $obj_b = $db->fetch_object($res_b);
        $cta_banco = ($obj_b) ? $obj_b->account_number : '111005';

        // 2. Obtener Próximo Asiento
        $res_max = $db->query("SELECT MAX(CAST(piece_num AS UNSIGNED)) as max_p FROM ".MAIN_DB_PREFIX."accounting_bookkeeping");
        $next_p = ($db->fetch_object($res_max)->max_p) + 1;

        // --- INICIO DE PROCESAMIENTO CON VALIDACIÓN ---
        ...
        if (!$res_bank) die("❌ ERROR EN TABLE 'bank': " . $db->lasterror() . "<br>SQL: " . $sql_bank);
        $fk_bank_line = $db->last_insert_id(MAIN_DB_PREFIX."bank");

        // B. INSERT DÉBITO (CONTABILIDAD)
        $sql_d = "INSERT INTO ".MAIN_DB_PREFIX."accounting_bookkeeping (entity, piece_num, doc_date, doc_type, doc_ref, fk_doc, numero_compte, debit, credit, montant, sens, fk_user_author, date_creation, code_journal, journal_label) 
                  VALUES (".$conf->entity.", $next_p, '".$mov->date_mvmt."', 'bank', 'EXT-".$id_extracto."', $fk_bank_line, '$cta_banco', $monto, 0, $monto, 'D', ".$user->id.", '$fecha_ahora', 'BANK', 'Ingresos Bancarios')";
        $res_d = $db->query($sql_d);
        if (!$res_d) die("❌ ERROR EN DÉBITO CONTABLE: " . $db->lasterror() . "<br>SQL: " . $sql_d);

        // C. INSERT CRÉDITO (CONTABILIDAD)
        $sql_c = "INSERT INTO ".MAIN_DB_PREFIX."accounting_bookkeeping (entity, piece_num, doc_date, doc_type, doc_ref, fk_doc, numero_compte, debit, credit, montant, sens, fk_user_author, date_creation, code_journal, journal_label) 
                  VALUES (".$conf->entity.", $next_p, '".$mov->date_mvmt."', 'bank', 'EXT-".$id_extracto."', $fk_bank_line, '421005', 0, $monto, $monto, 'C', ".$user->id.", '$fecha_ahora', 'BANK', 'Ingresos Bancarios')";
        $res_c = $db->query($sql_c);
        if (!$res_c) die("❌ ERROR EN CRÉDITO CONTABLE: " . $db->lasterror() . "<br>SQL: " . $sql_c);

        // D. UPDATE EXTRACTO
        $res_u = $db->query("UPDATE ".MAIN_DB_PREFIX."ofinova_extracto SET status = 1 WHERE rowid = ".$id_extracto);
        if (!$res_u) die("❌ ERROR EN UPDATE EXTRACTO: " . $db->lasterror());

        // SI LLEGÓ AQUÍ, ES PORQUE TODO SE GRABÓ EN EL DISCO
        die("✅ ÉXITO REAL: Movimiento bancario ID $fk_bank_line y Asiento #$next_p creados correctamente.");
    }
}
