...
// ✅ CAPTURA CORRECTA DE search_account COMO ENTERO DESDE EL INICIO
$search_ref = '';
$search_account = 0;

// Acción del botón de búsqueda: Resetear filtros de sesión
if (GETPOST('button_removefilter', 'alpha')) {
    unset($_SESSION[$contextpage]['search_ref']);
    unset($_SESSION[$contextpage]['search_account']);
    $search_ref = '';
    $search_account = 0;
} else {
    // ✅ CAPTURA CORRECTA: search_account como INT
    if (GETPOSTINT('search_account') > 0) {
        $search_account = GETPOSTINT('search_account');
        $_SESSION[$contextpage]['search_account'] = $search_account;
    } elseif (!empty($_SESSION[$contextpage]['search_account'])) {
        $search_account = (int)$_SESSION[$contextpage]['search_account'];
    }
    
    // Captura de search_ref como string
    if (GETPOST('search_ref', 'alpha') !== '') {
        $search_ref = GETPOST('search_ref', 'alpha');
        $_SESSION[$contextpage]['search_ref'] = $search_ref;
    } elseif (!empty($_SESSION[$contextpage]['search_ref'])) {
...
// ✅ 3. CONSULTA SQL PARA CONTAR TOTAL DE REGISTROS (SIN LIMIT)
$sql_count = "SELECT COUNT(DISTINCT b.rowid) as total ";
$sql_count .= " FROM " . MAIN_DB_PREFIX . "bank as b ";
$sql_count .= " INNER JOIN " . MAIN_DB_PREFIX . "bank_account as ba ON b.fk_account = ba.rowid ";
$sql_count .= " LEFT JOIN " . MAIN_DB_PREFIX . "accounting_journal as aj ON ba.fk_accountancy_journal = aj.rowid ";
...
// ✅ FILTRO CRÍTICO: Ahora search_account es INT y se compara correctamente
if ($search_account > 0) {
...
if ($resql_count) {
    ...
    $total_documentos = (int)$obj_count->total;
    $db->free($resql_count);
}
...
// ✅ 4. CONSTRUCCIÓN DE LA CONSULTA SQL CON LIMIT Y OFFSET (EFICIENTE)
...
// ✅ FILTRO CRÍTICO: Ahora search_account es INT
if ($search_account > 0) {
...
// ✅ LÍMITE Y OFFSET: PAGINACIÓN A NIVEL DE BD (CRÍTICO)
...
if ($resql) {
    ...
    print_barre_liste($langs->trans("Transacciones de Caja y Bancos por Asentar (Colombia)"), $page, $_SERVER["PHP_SELF"], '&search_ref='.urlencode($search_ref).'&search_account='.$search_account, $sortfield, $sortorder, '', -1, $total_documentos, '', 0, '', '', $limit, 1);
    ...
    if ($res_bancos_directo) {
        while ($banco_row = $db->fetch_object($res_bancos_directo)) {
            ...
            $selected_attr = ($search_account == (int)$banco_row->rowid) ? ' selected="selected"' : '';
